...
const previewState: MasterMutationState = {
  status: "error",
  message:
    "Предпросмотр не записывает данные. Для сохранения включите рабочий режим и PostgreSQL.",
  fieldErrors: {},
};
...
export async function createMasterAction(
  ...
  try {
    ...
    if (error instanceof MasterConflictError)
      return {
        status: "error",
        message: "Мастер с таким телефоном уже есть.",
        fieldErrors: { phone: ["Телефон уже используется"] },
      };
    unexpected("masters.create", member.memberId, error);
    return {
      status: "error",
      message: "Не удалось добавить мастера. Изменения не сохранены.",
      fieldErrors: {},
    };
  }
}
...
export async function updateMasterAction(
  ...
  try {
    ...
    return {
      status: "success",
      message: "Карточка мастера обновлена.",
      fieldErrors: {},
    };
  } catch (error) {
    if (error instanceof MasterConflictError)
    ...
    if (error instanceof MasterHasFutureVisitsError)
      return {
        status: "error",
        message:
          "У мастера есть будущие выезды. Сначала переназначьте или отмените их.",
        fieldErrors: { active: ["Деактивация недоступна"] },
      };
    if (error instanceof MasterVersionConflictError)
      return {
        status: "error",
        message:
          "Карточку уже изменил другой сотрудник. Закройте окно, обновите страницу и повторите.",
        fieldErrors: {},
      };
    if (error instanceof MasterNotFoundError)
      return {
        status: "error",
        message: "Мастер больше не существует или недоступен.",
        fieldErrors: {},
      };
    unexpected("masters.update", member.memberId, error);
    return {
      status: "error",
      message: "Не удалось обновить мастера. Изменения не сохранены.",
      fieldErrors: {},
    };
  }
}
