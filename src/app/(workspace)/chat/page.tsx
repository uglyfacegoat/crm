...
export default async function ChatPage({ searchParams }: { searchParams: Promise<{ channel?: string }> }) {
  ...
  return <div className="flex h-full min-h-0 flex-col"><PageHeading eyebrow="Коммуникации" title="Внутренний чат" description={member.role === "master" ? "Связь с офисом, файлы и история по выездам." : "Рабочие группы, личные сообщения и единая история общения."} action={canWrite ? <ChatCreationActions memberOptions={data.memberOptions} currentMemberId={currentMemberId} canManage={canManage} /> : undefined} /><ChatWorkspace data={data} canWrite={canWrite} canManage={canManage} composerRequestKey={randomUUID()} /></div>;
}
