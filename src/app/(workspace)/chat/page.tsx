...
export default async function ChatPage({ searchParams }: { searchParams: Promise<{ channel?: string }> }) {
  ...
  if (!canRead) return <div><PageHeading eyebrow="Коммуникации" title="Внутренний чат" description="Рабочие группы и история переписки офиса." /><section className="surface-panel mt-7 p-8 text-sm text-[var(--muted)]">Для этой роли внутренний чат пока недоступен.</section></div>;
