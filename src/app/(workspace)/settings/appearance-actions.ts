...
import { z } from "zod";
import { APPEARANCE_THEME_COOKIE, DIGIT_STYLE_COOKIE, FONT_SCALE_COOKIE, appearanceThemeOptions, digitStyleOptions, fontScaleOptions } from "@/lib/appearance";
import { requirePermission } from "@/server/auth/permissions";
import { requireSession } from "@/server/auth/session";

const appearanceSchema = z.object({
  theme: z.enum(appearanceThemeOptions),
...
export async function saveAppearanceAction(_state: AppearanceState, formData: FormData): Promise<AppearanceState> {
  ...
  const parsed = appearanceSchema.safeParse({ theme: formData.get("theme"), fontScale: formData.get("fontScale"), digitStyle: formData.get("digitStyle") });
  if (!parsed.success) return { status: "error", message: "Выберите допустимую тему, размер текста и вид цифр." };
  ...
  cookieStore.set(APPEARANCE_THEME_COOKIE, parsed.data.theme, options);
