...
function ActionStatus({ state }: { state: FinanceActionState }) {
  ...
  return <div role="status" className={`rounded-[12px] border p-3 text-xs leading-5 ${state.status === "success" ? "border-[#69d3a4]/20 bg-[#69d3a4]/[0.05] text-[#83d4b2]" : "border-[#ef646a]/20 bg-[#ef646a]/[0.05] text-[#df8c90]"}`}>{state.message}</div>;
}
...
function FormFooter({ pending, saved, onClose, submitLabel, danger = false }: { pending: boolean; saved: boolean; onClose: () => void; submitLabel: string; danger?: boolean }) {
  return <footer className="mt-auto flex shrink-0 items-center justify-end gap-2 border-t border-white/[0.07] bg-[#0d1317] p-4 sm:px-7"><button type="button" onClick={onClose} className="focus-ring h-11 rounded-[12px] border border-white/[0.08] px-4 text-xs text-[#9aa3a8]">Отмена</button><button disabled={pending || saved} className={`focus-ring flex h-11 min-w-36 items-center justify-center gap-2 rounded-[12px] px-4 text-xs font-semibold disabled:opacity-60 ${danger ? "bg-[#d6545a] text-white" : "bg-[var(--accent)] text-[#111509]"}`}>{pending ? <LoaderCircle className="size-4 animate-spin" /> : saved ? <Check className="size-4" /> : null}{saved ? "Сохранено" : submitLabel}</button></footer>;
}
...
function PayoutForm({ order, today, onClose }: { order: FinanceOrder; today: string; onClose: () => void }) {
  ...
  const [requestKey] = useState(() => crypto.randomUUID());
  useCloseOnSuccess(state, onClose);
  return <form action={action} className="flex min-h-0 flex-1 flex-col"><input type="hidden" name="idempotencyKey" value={requestKey} /><input type="hidden" name="orderId" value={order.id} /><div className="flex-1 space-y-5 p-5 sm:p-7"><div className="rounded-[14px] border border-[#9c82e8]/15 bg-[#9c82e8]/[0.035] p-4"><p className="text-xs font-medium text-white">{order.masterName} · {order.number}</p><p className="mt-1 text-[10px] text-[#7b758c]">{order.client} · {order.object}</p><div className="mt-4 flex items-center justify-between text-xs"><span className="text-[#888093]">К выплате</span><strong className="font-display text-[#b8a5eb]">{formatMoneyMinor(order.masterDueMinor)}</strong></div></div><div className="grid gap-4 sm:grid-cols-2"><OrderField label="Сумма, ₽" required errors={state.fieldErrors.amount}><input name="amount" required inputMode="decimal" defaultValue={order.masterDueMinor / 100} className={orderInputClass} /></OrderField><OrderField label="Дата выплаты" required errors={state.fieldErrors.paidOn}><input name="paidOn" type="date" required max={today} defaultValue={today} className={orderInputClass} /></OrderField></div><PaymentMethodField errors={state.fieldErrors.paymentMethod} /><OrderField label="Номер ведомости / операции" errors={state.fieldErrors.reference}><input name="reference" maxLength={200} className={orderInputClass} /></OrderField><OrderField label="Комментарий" errors={state.fieldErrors.note}><textarea name="note" maxLength={2000} className={orderTextareaClass} /></OrderField><ActionStatus state={state} /></div><FormFooter pending={pending} saved={state.status === "success"} onClose={onClose} submitLabel="Провести выплату" /></form>;
}
