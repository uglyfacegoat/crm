...
function ActionStatus({ state }: { state: FinanceActionState }) {
  ...
  return <div role="status" className={`rounded-[12px] border p-3 text-xs leading-5 ${state.status === "success" ? "border-[var(--success-border)] bg-[var(--success-bg)] text-[var(--success)]" : "border-[var(--danger-border)] bg-[var(--danger-bg)] text-[var(--danger-ink)]"}`}>{state.message}</div>;
}
...
function FormFooter({ pending, saved, onClose, submitLabel, danger = false }: { pending: boolean; saved: boolean; onClose: () => void; submitLabel: string; danger?: boolean }) {
  return <footer className="mt-auto flex shrink-0 items-center justify-end gap-2 border-t border-[var(--line)] bg-[var(--surface-raised)] p-4 sm:px-7"><button type="button" onClick={onClose} className="focus-ring h-11 rounded-[12px] border border-[var(--line-strong)] bg-[var(--surface)] px-4 text-xs text-[var(--text-secondary)] hover:bg-[var(--surface-soft)]">Отмена</button><button disabled={pending || saved} className={`focus-ring flex h-11 min-w-36 items-center justify-center gap-2 rounded-[12px] px-4 text-xs font-semibold disabled:opacity-60 ${danger ? "bg-[var(--danger)] text-[var(--on-accent)] hover:bg-[var(--danger-ink)]" : "bg-[var(--accent)] text-[var(--on-accent)] hover:bg-[var(--accent-strong)]"}`}>{pending ? <LoaderCircle className="size-4 animate-spin" /> : saved ? <Check className="size-4" /> : null}{saved ? "Сохранено" : submitLabel}</button></footer>;
}
...
function ReceiptUploadField({ errors }: { errors?: string[] }) {
  return <OrderField label="Чек или подтверждение" errors={errors}><label className="focus-within:ring-[var(--focus)] flex min-h-12 cursor-pointer items-center gap-3 rounded-[12px] border border-dashed border-[var(--line-strong)] bg-[var(--surface-inset)] px-4 text-xs text-[var(--text-secondary)] focus-within:ring-2"><Paperclip className="size-4 shrink-0 text-[var(--muted)]" /><span className="min-w-0 flex-1">Прикрепить PDF, изображение, DOCX или XLSX</span><input type="file" name="receipt" accept=".pdf,.jpg,.jpeg,.png,.webp,.docx,.xlsx" className="w-28 text-[10px] file:hidden" /></label><p className="mt-2 text-[9px] leading-4 text-[var(--muted)]">После проведения файл автоматически попадёт в папку «Чеки» в документах.</p></OrderField>;
}
