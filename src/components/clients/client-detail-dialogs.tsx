...
function FormFooter({ pending, saved, onCancel, submitLabel }: { pending: boolean; saved: boolean; onCancel: () => void; submitLabel: string }) {
  return <footer className="sticky bottom-0 flex gap-2 border-t border-white/[0.07] bg-[#0d1317]/94 p-4 backdrop-blur-xl sm:px-7"><button type="button" onClick={onCancel} disabled={pending} className="focus-ring h-12 flex-1 rounded-[12px] border border-white/[0.08] text-xs text-[#8b959b] hover:bg-white/[0.04]">Отмена</button><button type="submit" disabled={pending || saved} className="focus-ring flex h-12 flex-[1.4] items-center justify-center gap-2 rounded-[12px] bg-[var(--accent)] text-xs font-semibold text-[#111509] disabled:cursor-wait disabled:opacity-65">{pending ? <><LoaderCircle className="size-4 animate-spin" />Сохраняем…</> : saved ? <><Check className="size-4" />Сохранено</> : submitLabel}</button></footer>;
}
