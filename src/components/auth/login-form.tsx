  ...
  return <div className="mt-8 w-full min-w-0">
    <form action={formAction} noValidate>
      ...
      <div className="space-y-3">
        <label className="block text-left"><span className="mb-2 block text-[10px] font-medium text-[var(--text-secondary)]">Email или телефон</span><input required={!preview} name="identity" autoComplete="username" placeholder="Email или телефон" className="h-14 w-full border-b border-[var(--line-strong)] bg-transparent px-0 text-left text-sm text-[var(--text)] outline-none transition-colors placeholder:text-[var(--muted-subtle)] focus:border-[var(--accent)]" /></label>
        <label className="relative block pt-2 text-left"><span className="mb-2 block text-[10px] font-medium text-[var(--text-secondary)]">Пароль</span><input required={!preview} minLength={8} maxLength={128} name="password" autoComplete="current-password" type={showPassword ? "text" : "password"} placeholder="Пароль" className="h-14 w-full border-b border-[var(--line-strong)] bg-transparent pr-10 text-left text-sm text-[var(--text)] outline-none transition-colors placeholder:text-[var(--muted-subtle)] focus:border-[var(--accent)]" /><button type="button" onClick={() => setShowPassword((current) => !current)} aria-label={showPassword ? "Скрыть пароль" : "Показать пароль"} className="focus-ring absolute bottom-2 right-0 rounded-lg p-3 text-[var(--muted)] hover:text-[var(--text)]">{showPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></label>
      </div>
      <div className="mt-4 flex flex-col items-start gap-3 text-left text-xs max-[359px]:text-[10px]"><label className="flex items-center gap-2 text-[var(--text-secondary)]"><input type="checkbox" name="remember" className="size-4 accent-[var(--accent)]" />Запомнить меня</label><span className="text-[var(--muted)]">Восстановление — через администратора</span></div>
