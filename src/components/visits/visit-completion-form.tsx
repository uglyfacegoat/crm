    ...
    <div className="flex-1 space-y-5 overflow-y-auto p-5 sm:p-7">
      <section className="overflow-hidden rounded-[18px] border border-[var(--line-strong)] bg-[var(--accent-soft)] p-4 sm:p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-[var(--accent-ink)]">Контроль закрытия</p><h3 className="mt-2 truncate text-sm font-semibold text-[var(--text)]">{visit.client}</h3><p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">{visit.object} · {visitDate(visit)}</p></div>
        </div>
        <p className="mt-4 text-[10px] leading-5 text-[var(--text-secondary)]">Статус изменится только после сохранения проверенного файла. Акт автоматически попадёт в архив заказа и останется связан с этим выездом.</p>
      </section>
      ...
      <OrderField label="Подписанный акт" required errors={state.fieldErrors.file}>
        <label className="flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-[15px] border border-dashed border-[var(--line-strong)] bg-[var(--surface-inset)] px-4 py-5 text-center hover:border-[var(--accent)] hover:bg-[var(--accent-soft)] focus-within:border-[var(--accent)] focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-[var(--accent)]">
          ...
          {filename ? <><FileCheck2 className="size-6 text-[var(--success)]" /><span className="mt-2 max-w-full truncate text-xs text-[var(--text-secondary)]">{filename}</span><span className="mt-1 text-[9px] text-[var(--muted)]">Нажмите, чтобы заменить</span></> : <><Upload className="size-6 text-[var(--muted)]" /><span className="mt-2 text-xs text-[var(--text-secondary)]">Выбрать акт или фотографию</span><span className="mt-1 text-[9px] text-[var(--muted)]">PDF, JPG, PNG или WebP · до 15 МБ</span></>}
        </label>
      </OrderField>

      {state.message ? <div role={state.status === "error" ? "alert" : "status"} className={`flex items-start gap-2 rounded-[12px] border p-3 text-xs leading-5 ${state.status === "success" ? "border-[var(--success-border)] bg-[var(--success-bg)] text-[var(--success)]" : "border-[var(--danger-border)] bg-[var(--danger-bg)] text-[var(--danger-ink)]"}`}>{state.status === "success" ? <Check className="mt-0.5 size-4 shrink-0" /> : <AlertTriangle className="mt-0.5 size-4 shrink-0" />}{state.message}</div> : null}
    </div>
    <footer className="flex shrink-0 flex-col-reverse gap-2 border-t border-[var(--line)] bg-[var(--surface-raised)] p-4 sm:flex-row sm:justify-end sm:px-7">
      <button type="button" onClick={onClose} disabled={pending} className="focus-ring h-11 rounded-[12px] border border-[var(--line-strong)] px-5 text-xs text-[var(--text-secondary)] hover:bg-[var(--surface-soft)] disabled:opacity-50">Отмена</button>
      <button type="submit" disabled={pending || state.status === "success"} className="focus-ring flex h-11 items-center justify-center gap-2 rounded-[12px] bg-[var(--accent)] px-5 text-xs font-semibold text-[var(--on-accent)] disabled:opacity-55">{pending ? <><LoaderCircle className="size-4 animate-spin" />Проверяем и сохраняем…</> : state.status === "success" ? <><Check className="size-4" />Выезд завершён</> : <><FileCheck2 className="size-4" />Завершить с актом</>}</button>
    </footer>
  </form>;
}
