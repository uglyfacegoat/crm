...
const terminalStatuses = new Set<ServiceVisit["statusCode"]>([
  "completed",
  "cancelled",
]);
...
const eventColors: Record<ServiceVisit["statusCode"], string> = {
  planned:
    "border-[var(--line-strong)] bg-[var(--surface-raised)] text-[var(--text-secondary)]",
  confirmed:
    "border-[var(--support-strong)] bg-[var(--support-soft)] text-[var(--support-strong)]",
  in_progress:
    "border-[var(--accent-strong)] bg-[var(--accent-soft)] text-[var(--accent-ink)]",
  completed:
    "border-[var(--success-border)] bg-[var(--success-bg)] text-[var(--success)]",
  cancelled:
    "border-[var(--danger-border)] bg-[var(--danger-bg)] text-[var(--danger-ink)] opacity-70",
};
...
function localVisitEntry(
  visit: ServiceVisit,
): CalendarEntry & { date: string } {
  ...
  const endMinute =
    endDate === date ? Number(end.hour) * 60 + Number(end.minute) : 24 * 60;
  return {
    visit,
    date,
    ...
    >
      ...
      >
        <div className="flex-1 space-y-6 p-5 sm:p-7">
          <div
            className={`rounded-[14px] border p-4 ${eventColors[visit.statusCode]}`}
          >
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] opacity-75">
              {visit.orderNumber ?? "Без номера"}
            </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="grid gap-2 text-[10px] text-[var(--muted)]">
              ...
              <DateInput
                required
                name="localDate"
                defaultValue={draft.localDate}
                className="focus-ring h-12 rounded-[12px] border border-[var(--line)] bg-[var(--surface-inset)] px-3.5 text-sm text-[var(--text)] [color-scheme:light]"
              />
            </label>
            <label className="grid gap-2 text-[10px] text-[var(--muted)]">
          ...
          <p className="rounded-[12px] border border-[var(--line)] bg-[var(--surface-raised)] p-3 text-[10px] leading-5 text-[var(--muted)]">
            При переносе автоматически изменится задача-напоминание. Если мастер
            занят, система не сохранит конфликтующее время.
          </p>
          {error ? (
            <p
              role="alert"
        ...
        <footer className="sticky bottom-0 flex gap-2 border-t border-[var(--line)] bg-[var(--surface)] p-4 backdrop-blur-xl sm:px-7">
          <button
            type="button"
            onClick={onClose}
            disabled={pending}
            className="focus-ring h-12 flex-1 rounded-[12px] border border-[var(--line)] bg-[var(--surface-raised)] text-xs text-[var(--text-secondary)] hover:bg-[var(--surface-soft)] disabled:opacity-60"
          >
            Отмена
          </button>
          <button
            type="submit"
            disabled={pending}
            className="focus-ring flex h-12 flex-[1.4] items-center justify-center gap-2 rounded-[12px] bg-[var(--accent)] text-xs font-semibold text-[var(--on-accent)] hover:bg-[var(--accent-strong)] disabled:opacity-60"
          >
            {pending ? (
              <>
    ...
    <section className="surface-panel mt-3 overflow-hidden">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--line)] px-4 py-4 sm:px-6">
        <div>
          <h2 className="text-sm font-semibold text-[var(--text)]">
            Расписание всех заказов
          </h2>
          <p className="mt-1 text-xs text-[var(--muted)]">
            Каждый выезд хранится отдельно и ведёт в свою карточку заказа.
          </p>
        </div>
        <span className="rounded-full bg-[var(--surface-raised)] px-3 py-1.5 text-[10px] text-[var(--muted)]">
          {orderedVisits.length} выезд.
        </span>
      </header>
      {orderedVisits.length ? (
        <div className="divide-y divide-[var(--line)]">
          {orderedVisits.map((visit) => (
            ...
            >
              ...
              <span
                className={`w-fit rounded-full border px-2.5 py-1 text-[9px] ${eventColors[visit.statusCode]}`}
              >
                {visit.status}
              </span>
              <div className="flex items-center gap-2">
                ...
                {canWrite && !terminalStatuses.has(visit.statusCode) ? (
                  <button
                    ...
                    aria-label={`Перенести выезд ${visit.orderNumber ?? visit.client}`}
                    className="focus-ring grid size-9 place-items-center rounded-[10px] border border-[var(--line)] bg-[var(--surface-raised)] text-[var(--muted)] hover:bg-[var(--surface-soft)] hover:text-[var(--text)]"
                  >
                    <Move className="size-4" />
                  </button>
                ) : null}
        ...
        <div className="grid min-h-64 place-items-center text-center">
          <div>
            <CalendarDays className="mx-auto size-8 text-[var(--muted-subtle)]" />
            <p className="mt-3 text-xs text-[var(--muted)]">
              В выбранном периоде выездов нет
            </p>
          </div>
        </div>
      )}
    ...
    <aside className="surface-panel min-w-0 p-3 sm:p-4">
      <div className="flex items-center gap-3">
        <div className="min-w-0 flex-1">
          <h2 className="text-sm font-semibold text-[var(--text)]">
            Без даты / нужно назначить
          </h2>
          <p className="mt-1 text-[10px] text-[var(--muted)]">
            {orders.length} заказ. · перетащите в расписание
          </p>
        </div>
        <button
          type="button"
          onClick={onClose}
          aria-label="Скрыть неназначенные заказы"
          className="focus-ring grid size-9 place-items-center rounded-[10px] border border-[var(--line)] bg-[var(--surface-raised)] text-[var(--muted)] hover:bg-[var(--surface-soft)] hover:text-[var(--text)]"
        >
          <ChevronRight className="size-4" />
        </button>
      </div>
      <div className="mt-3 max-h-[calc(100dvh-20rem)] space-y-2 overflow-y-auto">
        {orders.length ? (
          orders.map((order) => (
            <article
              key={order.id}
              draggable={canWrite && pendingId !== order.id}
              onDragStart={(event) => onDragStart(order, event)}
              onDragEnd={onDragEnd}
              className={`group rounded-[12px] border border-[var(--line)] bg-[var(--surface-raised)] p-3 hover:border-[var(--accent)] hover:bg-[var(--accent-soft)] ${canWrite ? "cursor-grab active:cursor-grabbing" : ""} ${pendingId === order.id ? "opacity-45" : ""}`}
            >
              <div className="flex items-center justify-between gap-3">
                <strong className="font-display text-[10px] text-[var(--accent)]">
                  {order.number}
                </strong>
              ...
              <Link
                href={`/orders/${order.id}?newVisit=1`}
                className="focus-ring mt-2 block rounded"
              >
                <p className="truncate text-xs font-medium text-[var(--text)]">
                  {order.client}
                </p>
          ...
          <div className="grid min-h-48 place-items-center text-center">
            <div>
              <Check className="mx-auto size-7 text-[var(--success)]" />
              <p className="mt-3 text-xs text-[var(--text-secondary)]">
                Все активные заказы уже в расписании
              </p>
            </div>
          </div>
        )}
                  ...
                  <div className="flex shrink-0 gap-2">
                    ...
                    {canWrite && !terminalStatuses.has(visit.statusCode) ? (
                      <button
                        ...
                        onClick={() => onOpenMove(visit)}
                        aria-label={`Перенести выезд ${visit.orderNumber ?? visit.client}`}
                        className="focus-ring grid size-10 shrink-0 place-items-center rounded-[11px] border border-[var(--line)] bg-[var(--surface)] text-[var(--text-secondary)] hover:bg-[var(--surface-soft)] hover:text-[var(--text)]"
                      >
                        <Move className="size-4" />
                      </button>
                    ) : null}
          ...
          {days.map((day) => {
            ...
            return (
              <div
                ...
                role="group"
                aria-label={`Расписание на ${day.date}`}
                onDragOver={(event) => onDragOverDay(day.date, event)}
                onDrop={(event) => onDropDay(day.date, event)}
                className={`relative border-r border-[var(--line)] bg-[repeating-linear-gradient(to_bottom,transparent_0,transparent_65px,var(--line)_66px)] ${day.date === selectedDate ? "bg-[color-mix(in_srgb,var(--accent)_8%,var(--surface))]" : ""} ${draggingId ? "after:pointer-events-none after:absolute after:inset-1 after:rounded-xl after:border after:border-dashed after:border-[var(--accent)]" : ""}`}
              >
                {dropTarget?.date === day.date ? (
                  <div
                    className="pointer-events-none absolute left-0 right-0 z-30 border-t border-[var(--accent)]"
                    style={{
                      top:
                        ((dropTarget.minute - START_HOUR * 60) / 60) *
                        HOUR_HEIGHT,
                    }}
                  >
                    <span className="absolute -top-3 left-1 rounded-md bg-[var(--accent)] px-1.5 py-0.5 font-display text-[8px] font-semibold text-[var(--on-accent)]">
                      {formatTime(dropTarget.minute)}
                    </span>
                  </div>
                ) : null}
                {entries.map((entry) => {
                  ...
                  const top =
                    ((visibleStart - START_HOUR * 60) / 60) * HOUR_HEIGHT;
                  const height = Math.max(
                    22,
                    ((visibleEnd - visibleStart) / 60) * HOUR_HEIGHT - 3,
                  );
                  ...
                  const collapsedCluster =
                    position.laneCount > visibleLaneCount &&
                    position.lane === visibleLaneCount - 1;
                  ...
                  if (collapsedCluster)
                    return (
                      <article
                        key={`cluster-${entry.visit.id}`}
                        aria-label={`Ещё ${position.laneCount - visibleLaneCount + 1} выездов в этом интервале`}
                        className="absolute z-10 min-w-0 overflow-hidden rounded-md border border-[var(--line-strong)] bg-[var(--surface-raised)] px-1.5 py-2 text-[var(--text-secondary)]"
                        style={{
                          top,
                          height,
                        ...
                        title="Откройте режим списка, чтобы увидеть все выезды"
                      >
                        <span className="block truncate font-display text-xs font-semibold">
                          +{position.laneCount - visibleLaneCount + 1}
                        </span>
                        {height >= 40 ? (
                          <span className="mt-1 block truncate text-[8px]">
                            ещё выездов
                          </span>
                        ) : null}
                      </article>
                  ...
                  return (
                    <article
                      ...
                      onDragEnd={onDragEnd}
                      className={`absolute z-10 min-w-0 overflow-hidden border py-1.5 transition-[opacity,box-shadow] ${densityClass} ${eventColors[entry.visit.statusCode]} ${movable ? "cursor-grab active:cursor-grabbing" : ""} ${draggingId === entry.visit.id || pendingId === entry.visit.id ? "opacity-45" : "hover:z-20 hover:brightness-110"}`}
                      style={{
                        top,
                        height,
                        left: `calc(${position.lane * laneWidth}% + ${laneGap}px)`,
                    ...
                    >
                      ...
                      {height >= 36 && visibleLaneCount <= 5 ? (
                        ...
                        >
                          ...
                          {height >= 58 && visibleLaneCount <= 2 ? (
                            <p className="mt-1 truncate text-[8px] opacity-75">
                              {entry.visit.address}
                            </p>
                          ) : null}
                        </Link>
                      ) : null}
                      {height >= 62 && visibleLaneCount <= 2 ? (
                        <VisitDispatchCardButton
                          visitId={entry.visit.id}
                          compact
                          className="absolute bottom-1.5 left-1.5 size-5 rounded-md border-0"
                        />
                      ) : null}
                      {movable && height >= 62 && visibleLaneCount <= 2 ? (
                        <button
                          type="button"
                          onClick={() => onOpenMove(entry.visit)}
                          aria-label={`Точно перенести выезд ${entry.visit.orderNumber ?? entry.visit.client}`}
                          className="focus-ring absolute bottom-1.5 right-1.5 grid size-5 place-items-center rounded-md border border-[var(--line)] bg-[var(--surface)] text-[var(--text-secondary)] hover:bg-[var(--surface-soft)] hover:text-[var(--text)]"
                        >
...
export function CalendarWorkspace({
  ...
  const days = useMemo(
    () =>
      Array.from({ length: 7 }, (_, index) =>
        calendarDay(addDays(weekStart, index)),
      ),
    [weekStart],
  );
  const [selectedDate, setSelectedDate] = useState(() =>
    days.some((day) => day.date === anchorDate) ? anchorDate : days[0].date,
  );
  const masterOptions = useMemo(
    () => [
      { value: "all", label: "Все мастера" },
      { value: "unassigned", label: "Без мастера" },
      ...Array.from(
        new Set(
          calendarVisits.flatMap((visit) =>
            visit.master ? [visit.master] : [],
          ),
        ),
      )
  ...
  const regionOptions = useMemo(
    () => [
      ...
      ...Array.from(
        new Set(
          calendarVisits.flatMap((visit) =>
            visit.masterRegion ? [visit.masterRegion] : [],
          ),
        ),
      )
        .sort((left, right) => left.localeCompare(right, "ru"))
  ...
  const clientOptions = useMemo(
    () => [
      ...
      ...Array.from(new Set(calendarVisits.map((visit) => visit.client)))
        .sort((left, right) => left.localeCompare(right, "ru"))
        .map((name) => ({ value: name, label: name })),
    ],
    [calendarVisits],
  ...
  const objectOptions = useMemo(
    () => [
      ...
      ...Array.from(new Set(calendarVisits.map((visit) => visit.object)))
        .sort((left, right) => left.localeCompare(right, "ru"))
        .map((name) => ({ value: name, label: name })),
    ],
    [calendarVisits],
  ...
  const visibleVisits = useMemo(
    () =>
      filterCalendarVisits(calendarVisits, {
        query,
        master,
        region,
        client,
  ...
  const visitsByDate = useMemo(() => {
    ...
    for (const visit of visibleVisits) {
      ...
      grouped.set(
        entry.date,
        entries.toSorted((left, right) => left.startMinute - right.startMinute),
      );
    }
    return grouped;
  }, [visibleVisits]);
  ...
  const displayedDays =
    view === "day" ? [calendarDay(selectedDateValue)] : days;
  ...
  const monthDays = useMemo(
    () =>
      Array.from({ length: 42 }, (_, index) =>
        calendarDay(addDays(monthGridStart, index)),
      ),
    [monthGridStart],
  );
  const navigation = useMemo(() => {
    if (view === "day")
      return {
        previous: isoDate(addDays(selectedDateValue, -1)),
        next: isoDate(addDays(selectedDateValue, 1)),
    ...
    if (view === "month")
      return {
        previous: isoDate(addMonths(selectedDateValue, -1)),
        next: isoDate(addMonths(selectedDateValue, 1)),
        label: new Intl.DateTimeFormat("ru-RU", {
          month: "long",
          year: "numeric",
          timeZone: "UTC",
    ...
    return {
      previous: isoDate(addDays(weekStart, -7)),
      next: isoDate(addDays(weekStart, 7)),
      label: weekLabel(weekStart, addDays(weekStart, 6)),
      unit: "неделю",
    };
  ...
  function openMoveDialog(
    ...
    setMoveDraft({
      visit,
      localDate: localDate ?? current.date,
  ...
  function createVisitFromOrder(
    ...
    startTransition(async () => {
      ...
      const result = await createVisitAction(
        { status: "idle", message: null, fieldErrors: {}, visitId: null },
        formData,
      );
      if (result.status === "success") {
        ...
        setMessage({
          tone: "success",
          text: `Заказ ${order.number} добавлен в календарь на ${localDate}, ${localTime}.`,
        });
        router.refresh();
      } else {
        setMessage({
          tone: "error",
          text: result.message ?? "Не удалось добавить выезд.",
        });
      }
      setPendingId(null);
      setDraggingId(null);
  ...
  return (
    <div className="mt-[clamp(1.2rem,0.9rem+0.7vw,2rem)]">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex min-w-0 flex-wrap gap-2">
          ...
          <div className="flex items-center rounded-xl border border-[var(--line)] bg-[var(--surface)]">
            ...
            <span className="min-w-40 px-3 text-center font-display text-xs capitalize text-[var(--text)]">
              {navigation.label}
            </span>
            <Link
      ...
      <div className="mt-3 grid gap-2 lg:grid-cols-[minmax(16rem,1fr)_auto_auto] lg:items-start">
        <label
          data-calendar-filter="query"
          className="focus-within:border-[var(--accent)] flex h-11 min-w-0 items-center gap-2 rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] px-3 text-[var(--muted)]"
        >
          <Search className="size-4 shrink-0" />
          <span className="sr-only">Поиск по календарю</span>
          <input
            value={query}
        ...
        <details className="group relative">
          <summary className="focus-ring flex h-11 cursor-pointer list-none items-center justify-center gap-2 rounded-xl border border-[var(--line)] bg-[var(--surface)] px-4 text-xs text-[var(--text-secondary)] hover:bg-[var(--surface-raised)] hover:text-[var(--text)] [&::-webkit-details-marker]:hidden">
            ...
            Фильтры
            {activeFilterCount ? (
              <span className="grid size-5 place-items-center rounded-full bg-[var(--accent)] text-[9px] font-semibold text-[var(--on-accent)]">
                {activeFilterCount}
              </span>
            ) : null}
          </summary>
          <div className="absolute right-0 z-40 mt-2 w-[min(44rem,calc(100vw-2rem))] rounded-[16px] border border-[var(--line)] bg-[var(--surface-raised)] p-4 shadow-[var(--shadow-panel)] sm:p-5">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div data-calendar-filter="region">
                <OrderPicker
                  label="Регион"
                  value={region}
                  onChange={setRegion}
              ...
              <div data-calendar-filter="client">
                <OrderPicker
                  label="Клиент"
                  value={client}
                  onChange={setClient}
              ...
              <div data-calendar-filter="object">
                <OrderPicker
                  label="Объект"
                  value={objectFilter}
              ...
              <div data-calendar-filter="service">
                <OrderPicker
                  label="Услуга"
                  value={service}
                  onChange={setService}
            ...
            <div className="mt-4 flex items-center justify-between gap-3 border-t border-[var(--line)] pt-4">
              <p className="text-[10px] text-[var(--muted)]">
                Показано {visibleVisits.length} из {calendarVisits.length}{" "}
                выездов
              </p>
              <button
                type="button"
                ...
                className="focus-ring flex h-9 items-center gap-2 rounded-lg border border-[var(--line)] bg-[var(--surface)] px-3 text-[10px] text-[var(--text-secondary)] hover:bg-[var(--surface-soft)] hover:text-[var(--text)] disabled:cursor-not-allowed disabled:opacity-35"
              >
        ...
        <div className="flex h-11 items-center justify-center gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface)] px-3">
          <span className="text-[10px] text-[var(--muted)]">
            {visibleVisits.length} из {calendarVisits.length}
          </span>
          {canWrite ? (
            <span className="hidden items-center gap-1.5 text-[9px] text-[var(--muted)] lg:flex">
      ...
      {message ? (
        <p
          ...
          className={`mt-3 flex items-center gap-2 rounded-[12px] border p-3 text-xs ${message.tone === "success" ? "border-[var(--success-border)] bg-[var(--success-bg)] text-[var(--success)]" : "border-[var(--danger-border)] bg-[var(--danger-bg)] text-[var(--danger-ink)]"}`}
        >
        ...
        >
          <section className="surface-panel p-3 sm:p-4">
            <div className="grid grid-cols-7 gap-px overflow-hidden rounded-xl bg-[var(--line)]">
              {monthDays.map((day) => {
                const count = visitsByDate.get(day.date)?.length ?? 0;
                const inMonth = day.date.startsWith(selectedDate.slice(0, 7));
                return (
                  <button
                    key={day.date}
                    onClick={() => {
                  ...
                  >
                    ...
                    {count ? (
                      <span className="mt-3 block rounded-lg bg-[var(--accent-soft)] px-2 py-1 text-[9px] text-[var(--accent-ink)]">
                        {count} выезд.
                      </span>
                    ) : null}
                  </button>
      ...
      <MoveVisitDialog
        key={
          moveDraft
            ? `${moveDraft.visit.id}:${moveDraft.localDate}:${moveDraft.localTime}`
            : "closed"
        }
        draft={moveDraft}
        pending={isPending}
        ...
        onClose={() => {
          if (!isPending) setMoveDraft(null);
        }}
        onMove={moveVisit}
      />
    </div>
