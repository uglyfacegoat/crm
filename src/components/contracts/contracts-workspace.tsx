...
const quickFilters: Array<{ value: QuickFilter; label: string; tone: string }> = [
  { value: "all", label: "Все договоры", tone: "#b8f7e4" }, { value: "active", label: "Действуют", tone: "#b8f7e4" },
  { value: "expiring", label: "Требуют продления", tone: "#efb454" }, { value: "scheduled", label: "С плановыми выездами", tone: "#b8f7e4" },
];
const statusPresentation = {
  draft: { label: "Черновик", className: "border-white/[0.1] bg-white/[0.04] text-[#9da5aa]" },
  active: { label: "Действует", className: "border-[#b8f7e4]/25 bg-[#b8f7e4]/[0.07] text-[#78d4aa]" },
  suspended: { label: "Приостановлен", className: "border-[#efb454]/25 bg-[#efb454]/[0.07] text-[#e8b666]" },
  completed: { label: "Завершён", className: "border-[#b8f7e4]/20 bg-[#b8f7e4]/[0.06] text-[#b8f7e4]" },
...
export function ContractsWorkspace({ snapshot, canWrite }: { snapshot: ContractSnapshot; canWrite: boolean }) {
  ...
  return <div className="mt-[clamp(1.5rem,1.1rem+0.8vw,2.25rem)] space-y-4">
    <section aria-label="Быстрые фильтры договоров" className="flex gap-1 overflow-x-auto rounded-[14px] border border-white/[0.07] bg-white/[0.018] p-1.5">{quickFilters.map((entry) => <button key={entry.value} type="button" onClick={() => setQuickFilter(entry.value)} aria-pressed={quickFilter === entry.value} className={`focus-ring flex min-h-10 shrink-0 items-center gap-2 rounded-[10px] px-3 text-xs transition-colors ${quickFilter === entry.value ? "bg-white/[0.08] text-white" : "text-[#758087] hover:bg-white/[0.035] hover:text-white"}`}><span className="size-1.5 rounded-full" style={{ backgroundColor: entry.tone }} />{entry.label}<span className={`rounded-full px-1.5 py-0.5 font-display text-[9px] ${quickFilter === entry.value ? "bg-black/15 text-white" : "bg-white/[0.05] text-[#7d878d]"}`}>{counts[entry.value]}</span></button>)}</section>
