...
export function Dialog({ open, onClose, title, description, children }: { open: boolean; onClose: () => void; title: string; description?: string; children: React.ReactNode }) {
  ...
  if (!open) return null;
  return createPortal(<div className="fixed inset-0 z-[70] overflow-hidden bg-black/72 backdrop-blur-sm" onMouseDown={onClose} role="presentation"><div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={description ? descriptionId : undefined} onInputCapture={(event) => {
