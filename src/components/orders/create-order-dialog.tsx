...
function CreateOrderForm({ requestKey, options, onComplete }: { requestKey: string; options: OrderCreationOptions; onComplete: () => void }) {
  ...
  return <form action={action} className="flex flex-1 flex-col"><input type="hidden" name="idempotencyKey" value={requestKey} /><input type="hidden" name="clientId" value={clientId} /><input type="hidden" name="objectId" value={objectId} /><input type="hidden" name="contactId" value={contactId} /><input type="hidden" name="assignedMasterId" value={masterId} /><input type="hidden" name="services" value={JSON.stringify(serializedServices)} /><input type="hidden" name="expenses" value={JSON.stringify(serializedExpenses)} />
    <div className="flex-1 space-y-7 p-5 sm:p-7">
      ...
      <fieldset className="space-y-4"><legend className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Заказчик и объект</legend><OrderPicker label="Клиент" required value={clientId} onChange={chooseClient} options={options.clients.map((client) => ({ value: client.id, label: client.name }))} placeholder="Выберите клиента" searchable searchPlaceholder="Название или ИНН" errors={state.fieldErrors.clientId} /><OrderPicker label="Объект" required value={objectId} onChange={setObjectId} disabled={!clientId} options={objects.map((object) => ({ value: object.id, label: object.name, detail: object.address }))} placeholder={clientId ? "Выберите объект" : "Сначала выберите клиента"} searchable searchPlaceholder="Название или адрес" errors={state.fieldErrors.objectId} /><OrderPicker label="Контакт" required value={contactId} onChange={setContactId} disabled={!clientId} options={contacts.map((contact) => ({ value: contact.id, label: contact.name, detail: contact.phone }))} placeholder={clientId ? "Выберите контакт" : "Сначала выберите клиента"} searchable searchPlaceholder="Имя или телефон" errors={state.fieldErrors.contactId} /></fieldset>
      ...
      <fieldset className="space-y-4"><legend className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Исполнитель</legend><OrderPicker label="Мастер" value={masterId} onChange={setMasterId} options={[{ value: "", label: "Не назначен" }, ...options.masters.map((master) => ({ value: master.id, label: master.name, detail: master.phone }))]} placeholder="Не назначен" searchable searchPlaceholder="ФИО или телефон" errors={state.fieldErrors.assignedMasterId} /><OrderField label="Выплата мастеру, ₽" required={Boolean(masterId)} errors={state.fieldErrors.masterPayment}><input name="masterPayment" disabled={!masterId} required={Boolean(masterId)} inputMode="decimal" placeholder="4 000" className={orderInputClass} /></OrderField><OrderField label="Внутренняя заметка" errors={state.fieldErrors.notes}><textarea name="notes" maxLength={4000} placeholder="Условия заказа, особенности объекта, что проверить…" className={orderTextareaClass} /></OrderField></fieldset>
