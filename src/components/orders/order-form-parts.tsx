...
export function OrderPicker({ label, value, options, onChange, placeholder, disabled, required, errors }: {
  label: string;
  value: string;
...
export function OrderFormFooter({ pending, saved, onCancel, submitLabel, disabled }: { pending: boolean; saved: boolean; onCancel: () => void; submitLabel: string; disabled?: boolean }) {
  return <footer className="mt-auto flex shrink-0 gap-2 border-t border-white/[0.07] bg-[#0d1317] p-4 sm:px-7"><button type="button" onClick={onCancel} disabled={pending} className="focus-ring h-12 flex-1 rounded-[12px] border border-white/[0.08] text-xs text-[#8b959b] hover:bg-white/[0.04]">Отмена</button><button type="submit" disabled={pending || saved || disabled} className="focus-ring flex h-12 flex-[1.4] items-center justify-center gap-2 rounded-[12px] bg-[var(--accent)] text-xs font-semibold text-[#111509] disabled:cursor-not-allowed disabled:opacity-50">{pending ? <><LoaderCircle className="size-4 animate-spin" />Сохраняем…</> : saved ? <><Check className="size-4" />Сохранено</> : submitLabel}</button></footer>;
}
