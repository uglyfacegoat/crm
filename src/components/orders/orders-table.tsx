...
export function OrdersTable({ orders }: { orders: OrderListItem[] }) {
  ...
  const filteredOrders = useMemo(() => {
    ...
    const visible = orders.filter((order) => {
      ...
      return !normalizedQuery || [order.number, order.client, order.object, order.address, order.serviceSummary, order.master ?? ""].some((value) => value.toLocaleLowerCase("ru").includes(normalizedQuery));
    });
  ...
  return <div className="animate-rise" style={{ animationDelay: "100ms" }}>
    <section className="surface-panel overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-white/[0.06] p-3.5 min-[420px]:p-4 lg:flex-row lg:items-center lg:justify-between sm:px-5 2xl:py-5">
        <label className="soft-button flex h-10 min-w-0 items-center gap-2 rounded-[13px] px-3 sm:w-80 2xl:w-96"><Search className="size-4 shrink-0 text-[#707980]" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-[#5e676d]" placeholder="Номер, клиент, адрес, услуга или мастер" /></label>
        <div className="flex min-w-0 items-center gap-1.5 overflow-x-auto pb-1 sm:gap-2 sm:pb-0">
          <button type="button" onClick={openAdvancedFilters} className={`focus-ring flex h-10 shrink-0 items-center gap-2 rounded-[11px] border px-3 text-xs ${advancedCount ? "border-[var(--accent)]/25 bg-[var(--accent)]/[0.06] text-white" : "border-white/[0.07] text-[#858f94] hover:text-white"}`}><SlidersHorizontal className="size-4" />Фильтры{advancedCount ? <span className="grid min-w-5 place-items-center rounded-full bg-[var(--accent)] px-1.5 py-0.5 text-[9px] font-semibold text-[#25272c]">{advancedCount}</span> : null}</button>
          {totalFilterCount ? <button type="button" onClick={resetFilters} className="focus-ring flex h-10 shrink-0 items-center gap-2 rounded-[11px] border border-white/[0.07] px-3 text-xs text-[#7c858b] hover:text-white"><RotateCcw className="size-3.5" />Сбросить</button> : null}
          <div className="ml-auto flex shrink-0 rounded-[11px] border border-white/[0.07] p-1"><button type="button" onClick={() => setView("list")} aria-label="Показать списком" aria-pressed={view === "list"} className={`grid size-8 place-items-center rounded-[8px] ${view === "list" ? "bg-[var(--accent)] text-[#25272c]" : "text-[#707a80]"}`}><List className="size-4" /></button><button type="button" onClick={() => setView("grid")} aria-label="Показать карточками" aria-pressed={view === "grid"} className={`grid size-8 place-items-center rounded-[8px] ${view === "grid" ? "bg-[var(--accent)] text-[#25272c]" : "text-[#707a80]"}`}><Grid2X2 className="size-4" /></button></div>
        </div>
      </div>
      <div className="flex gap-1.5 overflow-x-auto border-b border-white/[0.055] bg-black/10 px-4 py-3 [scrollbar-width:none] sm:px-5 [&::-webkit-scrollbar]:hidden" aria-label="Фильтр по статусу">{statusOptions.map((option) => <button key={option} type="button" onClick={() => setStatus(option)} aria-pressed={status === option} className={`focus-ring flex h-9 shrink-0 items-center gap-2 rounded-[10px] px-3 text-[10px] font-medium transition-colors ${status === option ? "bg-white/[0.09] text-white ring-1 ring-white/[0.06]" : "text-[#737d83] hover:bg-white/[0.035] hover:text-white"}`}>{option}<span className={status === option ? "text-[var(--accent)]" : "text-[#505a60]"}>{statusCounts.get(option)}</span></button>)}</div>
      <div className={`${view === "list" ? "hidden lg:block" : "hidden"} overflow-x-auto`}><table className="w-full min-w-[900px] text-left"><thead className="text-[10px] uppercase tracking-[0.12em] text-[#626b71]"><tr><th className="px-5 py-3 font-medium">№ заказа</th><th className="px-4 py-3 font-medium">Клиент</th><th className="px-4 py-3 font-medium">Объект</th><th className="px-4 py-3 font-medium">Дата</th><th className="px-4 py-3 font-medium">Мастер</th><th className="px-4 py-3 font-medium">Статус</th><th className="px-5 py-3 text-right font-medium">Сумма</th></tr></thead><tbody className="divide-y divide-white/[0.05]">{filteredOrders.map((order) => <tr key={order.id} role="link" tabIndex={0} aria-label={`Открыть заказ ${order.number}`} onClick={() => router.push(`/orders/${order.id}`)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); router.push(`/orders/${order.id}`); } }} className="group cursor-pointer transition-colors hover:bg-white/[0.035] focus-visible:bg-white/[0.035] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--accent)] focus-visible:outline-offset-[-2px]"><td className="px-5 py-4"><Link href={`/orders/${order.id}`} className="focus-ring rounded font-display text-[11px] font-semibold text-white transition-colors hover:text-[var(--accent)]">{order.number}</Link></td><td className="px-4 py-4 text-xs font-medium text-[#dfe3df]">{order.client}</td><td className="px-4 py-4"><p className="text-xs text-[#a6aeb2]">{order.object}</p><p className="mt-1 text-[10px] text-[#697278]">{order.address}</p></td><td className="px-4 py-4 text-xs text-[#8c959a]">{formatShortDate(order.createdAt)}</td><td className={`px-4 py-4 text-xs ${order.master ? "text-[#a6aeb2]" : "text-[var(--danger)]"}`}>{order.master ?? "Не назначен"}</td><td className="px-4 py-4"><StatusBadge status={order.status} /></td><td className="px-5 py-4 text-right font-display text-[11px] text-white">{formatMoneyMinor(order.agreedTotalMinor)}</td></tr>)}</tbody></table></div>
      <div className={`${view === "list" ? "divide-y divide-white/[0.055] lg:hidden" : "grid gap-px bg-white/[0.055] sm:grid-cols-2 xl:grid-cols-3"}`}>{filteredOrders.map((order) => <Link key={order.id} href={`/orders/${order.id}`} className="focus-ring block bg-[var(--surface)] p-4 transition-colors duration-200 hover:bg-white/[0.035] sm:px-5"><div className="flex items-start justify-between gap-3"><div><p className="font-display text-[11px] font-semibold text-white">{order.number}</p><p className="mt-2 text-sm font-medium text-[#e0e4df]">{order.client}</p><p className="mt-1 text-xs text-[#747d83]">{order.object} · {order.address}</p></div><ChevronRight className="mt-1 size-4 text-[#596268]" /></div><div className="mt-4 flex items-center justify-between gap-2"><StatusBadge status={order.status} /><span className="font-display text-xs text-white">{formatMoneyMinor(order.agreedTotalMinor)}</span></div></Link>)}</div>
      {!filteredOrders.length ? <div className="px-5 py-16 text-center"><p className="text-sm font-medium text-white">Заказы не найдены</p><p className="mt-2 text-xs text-[#747d83]">Измените условия или сбросьте фильтры.</p><button type="button" onClick={resetFilters} className="focus-ring mt-4 rounded-[10px] bg-white/[0.07] px-3 py-2 text-xs text-white hover:bg-white/[0.1]">Сбросить фильтры</button></div> : null}
      <div className="flex items-center justify-between gap-3 border-t border-white/[0.06] px-4 py-3.5 text-[11px] text-[#747d83] sm:px-5 sm:text-xs"><span>Показано {filteredOrders.length} из {orders.length}</span><span className="text-right">{totalFilterCount ? `${totalFilterCount} активных условий` : "Без ограничений"}</span></div>
    </section>
    ...
    <Dialog open={advancedOpen} onClose={() => setAdvancedOpen(false)} title="Фильтры заказов" description="Сузьте список по назначению, мастеру, периоду, сумме и порядку отображения.">
      <div className="space-y-7 p-5 sm:p-7">
        <fieldset><legend className="mb-3 text-[10px] font-semibold uppercase tracking-[0.13em] text-[#69737a]">Назначение</legend><div className="grid gap-2 sm:grid-cols-3" role="radiogroup">{assignmentOptions.map((option) => <FilterChoice key={option.value} value={option.value} current={draft.assignment} label={option.label} onChange={(assignment) => setDraft((current) => ({ ...current, assignment, master: assignment === "unassigned" ? "" : current.master }))} />)}</div></fieldset>
        <fieldset><legend className="mb-3 text-[10px] font-semibold uppercase tracking-[0.13em] text-[#69737a]">Мастер</legend><div className="max-h-48 space-y-1 overflow-y-auto rounded-[13px] border border-white/[0.07] bg-black/10 p-1.5" role="radiogroup"><FilterChoice value="" current={draft.master} label="Любой мастер" onChange={(master) => setDraft((current) => ({ ...current, master }))} />{masters.map((master) => <FilterChoice key={master} value={master} current={draft.master} label={master} onChange={(selectedMaster) => setDraft((current) => ({ ...current, master: selectedMaster, assignment: "assigned" }))} />)}</div></fieldset>
        <fieldset><legend className="mb-3 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.13em] text-[#69737a]"><CalendarRange className="size-3.5" />Дата создания</legend><div className="grid gap-3 sm:grid-cols-2"><label className="block"><span className="mb-2 block text-[10px] text-[#747e84]">С даты</span><input inputMode="numeric" value={draft.dateFrom} onChange={(event) => setDraft((current) => ({ ...current, dateFrom: formatDateInput(event.target.value) }))} placeholder="ДД.ММ.ГГГГ" className="h-11 w-full rounded-[12px] border border-white/[0.08] bg-black/10 px-3 text-sm text-white outline-none focus:border-[var(--accent)]/45" /></label><label className="block"><span className="mb-2 block text-[10px] text-[#747e84]">По дату</span><input inputMode="numeric" value={draft.dateTo} onChange={(event) => setDraft((current) => ({ ...current, dateTo: formatDateInput(event.target.value) }))} placeholder="ДД.ММ.ГГГГ" className="h-11 w-full rounded-[12px] border border-white/[0.08] bg-black/10 px-3 text-sm text-white outline-none focus:border-[var(--accent)]/45" /></label></div></fieldset>
        <fieldset><legend className="mb-3 text-[10px] font-semibold uppercase tracking-[0.13em] text-[#69737a]">Согласованная сумма</legend><div className="grid gap-3 sm:grid-cols-2"><label className="block"><span className="mb-2 block text-[10px] text-[#747e84]">От, ₽</span><input inputMode="decimal" value={draft.amountMin} onChange={(event) => setDraft((current) => ({ ...current, amountMin: event.target.value }))} placeholder="0" className="h-11 w-full rounded-[12px] border border-white/[0.08] bg-black/10 px-3 text-sm text-white outline-none focus:border-[var(--accent)]/45" /></label><label className="block"><span className="mb-2 block text-[10px] text-[#747e84]">До, ₽</span><input inputMode="decimal" value={draft.amountMax} onChange={(event) => setDraft((current) => ({ ...current, amountMax: event.target.value }))} placeholder="Без ограничения" className="h-11 w-full rounded-[12px] border border-white/[0.08] bg-black/10 px-3 text-sm text-white outline-none focus:border-[var(--accent)]/45" /></label></div></fieldset>
        <fieldset><legend className="mb-3 text-[10px] font-semibold uppercase tracking-[0.13em] text-[#69737a]">Сортировка</legend><div className="grid gap-2 sm:grid-cols-2" role="radiogroup">{sortOptions.map((option) => <FilterChoice key={option.value} value={option.value} current={draft.sort} label={option.label} onChange={(sort) => setDraft((current) => ({ ...current, sort }))} />)}</div></fieldset>
