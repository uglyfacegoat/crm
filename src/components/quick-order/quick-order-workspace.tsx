...
export function QuickOrderWorkspace({ options, idempotencyKey, defaultVisitDate }: {
  ...
  const stepValid = [
    clientMode === "new"
      ? clientName.trim().length >= 2 && contactName.trim().length >= 2 && contactPhone.trim().length >= 7 && (clientKind === "individual" || /^\d{10}(\d{2})?$/.test(taxId))
      : Boolean(clientId) && (effectiveContactMode === "existing" ? Boolean(contactId) : contactName.trim().length >= 2 && contactPhone.trim().length >= 7),
  ...
  >
    ...
    <ol aria-label="Этапы оформления" className="mb-4 grid grid-cols-4 gap-1.5 sm:gap-2">
      {steps.map((entry, index) => { const Icon = entry.icon; const active = index === step; const done = index < step; return <li key={entry.title}><button type="button" onClick={() => { if (index <= step) setStep(index); }} disabled={index > step} aria-current={active ? "step" : undefined} className={`focus-ring flex min-h-14 w-full min-w-0 items-center justify-center gap-2 rounded-[14px] border px-1.5 transition-colors sm:justify-start sm:px-3 ${active ? "border-[var(--accent)]/25 bg-[var(--accent)]/[0.07] text-white" : done ? "border-[#69d3a4]/15 bg-[#69d3a4]/[0.035] text-[#9ba5a9]" : "border-white/[0.055] bg-white/[0.018] text-[#4e585e]"}`}><span className={`grid size-7 shrink-0 place-items-center rounded-[9px] ${active ? "bg-[var(--accent)] text-[#111509]" : done ? "bg-[#69d3a4]/10 text-[#69d3a4]" : "bg-white/[0.035]"}`}>{done ? <Check className="size-3.5" /> : <Icon className="size-3.5" />}</span><span className="hidden truncate text-[10px] font-medium min-[420px]:block">{entry.title}</span></button></li>; })}
    </ol>

    <section className="surface-panel min-h-[28rem] p-4 sm:p-6 lg:p-8">
      {step === 0 ? <>
