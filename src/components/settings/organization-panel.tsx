...
import {
  ...
  type OrganizationMutationState,
...
const initialState: OrganizationMutationState = {
  status: "idle",
  message: null,
  fieldErrors: {},
};
const organizationSwitchInitialState: OrganizationSwitchState = {
  status: "idle",
  message: null,
};
      ...
      <div className="flex-1 space-y-5 p-5 sm:p-7">
        ...
        >
          <input
            ...
            placeholder="Например, ТехСтройИнвест"
          />
        </OrderField>
        <OrderField
          label="Рабочий часовой пояс"
          required
        ...
        >
          <CustomSelect
            ...
            ariaLabel="Рабочий часовой пояс"
            options={timezoneOptions}
            className={orderInputClass}
          />
        </OrderField>
        <div className="flex gap-3 rounded-[13px] border border-[var(--info-border)] bg-[var(--info-bg)] p-4">
          <ShieldCheck className="mt-0.5 size-4 shrink-0 text-[var(--info)]" />
          <p className="text-[10px] leading-5 text-[var(--info)]">
            Компания получает отдельное рабочее пространство. Клиенты, заказы,
            финансы и документы не смешиваются с другими компаниями.
          </p>
        </div>
        {state.message ? (
          <p
...
function OpenOrganizationButton({
  organization,
}: {
  ...
  const [state, action, pending] = useActionState(
    switchOrganizationAction,
    organizationSwitchInitialState,
  );
  if (organization.current)
    return (
      <span className="rounded-full bg-[var(--success-bg)] px-2 py-1 text-[9px] text-[var(--success)]">
        Открыта
      </span>
    );
  return (
    <form action={action} className="text-right">
      ...
      <button
        type="submit"
        disabled={pending}
        className="focus-ring flex h-9 items-center gap-2 rounded-[10px] border border-[var(--line)] px-3 text-[10px] text-[var(--text-secondary)] hover:bg-[var(--surface-soft)] disabled:opacity-55"
      >
      ...
      {state.status === "error" && state.message ? (
        <p
          role="status"
          className="mt-2 max-w-40 text-[9px] leading-4 text-[var(--danger-ink)]"
        >
          {state.message}
        </p>
      ) : null}
    ...
    () =>
      organizations.reduce(
        (summary, organization) => ({
          ...
          receivedMinor:
            summary.receivedMinor === null ||
            organization.receivedMinor === null
              ? null
              : summary.receivedMinor + organization.receivedMinor,
        }),
        {
          clients: 0,
          orders: 0,
          ...
          visits: 0,
          tasks: 0,
          receivedMinor: 0 as number | null,
        },
      ),
    [organizations],
  );
  ...
  return (
    <div className="mt-5 space-y-4">
      <section className="surface-panel flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
        <span className="grid size-12 shrink-0 place-items-center rounded-[14px] bg-[var(--accent)]/[0.08] text-[var(--accent)]">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 max-w-2xl text-xs leading-5 text-[var(--muted)]">
            Центр показывает общую картину, а переход в компанию открывает
            только её клиентов, заказы, финансы и документы.
          </p>
        </div>
        <button
          type="button"
          ...
          className="focus-ring flex min-h-11 items-center justify-center gap-2 rounded-[13px] bg-[var(--accent)] px-4 text-xs font-semibold text-[var(--on-accent)] disabled:cursor-not-allowed disabled:opacity-40"
        >
          ...
          <div className="grid gap-px bg-[var(--line)] sm:grid-cols-3 xl:grid-cols-6">
            {[
              ...
              },
              {
                ...
                label: "Получено",
                value:
                  totals.receivedMinor === null
                    ? "Скрыто правами"
                    : formatMoneyMinor(totals.receivedMinor),
                tone: "text-[var(--accent)]",
              },
            ].map(({ icon: Icon, label, value, tone }) => (
              <div key={label} className="bg-[var(--surface-raised)] p-4">
                <Icon className={`size-4 ${tone}`} />
                <p className="mt-4 text-[9px] uppercase tracking-[0.12em] text-[var(--muted)]">
                  {label}
                </p>
          ...
          >
            <div className="p-5">
              <div className="flex items-start justify-between gap-3">
                <span
                  className={`grid size-10 place-items-center rounded-[12px] ${organization.kind === "center" ? "bg-[var(--accent-soft)] text-[var(--accent-ink)]" : "bg-[var(--support-soft)] text-[var(--support-strong)]"}`}
                >
                ...
                <OpenOrganizationButton organization={organization} />
              </div>
              <h3 className="mt-4 text-sm font-medium text-[var(--text)]">
                {organization.name}
              </h3>
      ...
      {!canCreate && current?.kind === "company" ? (
        <p className="text-[10px] text-[var(--muted)]">
          ...
          , чтобы добавить ещё одну компанию.
        </p>
      ) : null}
      <Dialog
        ...
        description="Создание отдельного рабочего пространства без смешивания данных."
      >
        {requestKey ? (
          <CreateOrganizationForm requestKey={requestKey} onComplete={close} />
        ) : null}
      </Dialog>
    </div>
  );
