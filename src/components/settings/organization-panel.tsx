...
function CreateOrganizationForm({ requestKey, onComplete }: { requestKey: string; onComplete: () => void }) {
  ...
  return <form action={action} className="flex min-h-full flex-1 flex-col">
    ...
    <div className="flex-1 space-y-5 p-5 sm:p-7">
      ...
      <div className="flex gap-3 rounded-[13px] border border-[#b8f7e4]/15 bg-[#b8f7e4]/[0.04] p-4"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-[#b8f7e4]" /><p className="text-[10px] leading-5 text-[#7892a1]">Компания получает отдельное рабочее пространство. Клиенты, заказы, финансы и документы не смешиваются с другими компаниями.</p></div>
      {state.message ? <p role="status" className={`rounded-[12px] border p-3 text-xs ${state.status === "success" ? "border-[#b8f7e4]/20 text-[#83d4b1]" : "border-[#ef646a]/20 text-[#dc8b90]"}`}>{state.status === "success" ? <Check className="mr-2 inline size-4" /> : null}{state.message}</p> : null}
    </div>
...
function OpenOrganizationButton({ organization }: { organization: OrganizationSummary }) {
  ...
  if (organization.current) return <span className="rounded-full bg-[#b8f7e4]/[0.08] px-2 py-1 text-[9px] text-[#78cfa8]">Открыта</span>;
...
export function OrganizationPanel({ organizations, preview }: { organizations: OrganizationSummary[]; preview: boolean }) {
  ...
  return <div className="mt-5 space-y-4">
    ...
    {current?.kind === "center" ? <section className="surface-panel overflow-hidden"><div className="border-b border-white/[0.06] px-5 py-4"><p className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[var(--accent)]">Сводка группы</p><h3 className="mt-1 text-sm font-semibold text-white">Все доступные компании</h3></div><div className="grid gap-px bg-white/[0.055] sm:grid-cols-3 xl:grid-cols-6">{[
      { icon: UsersRound, label: "Клиенты", value: totals.clients, tone: "text-[#b8f7e4]" },
      { icon: ClipboardList, label: "Все заказы", value: totals.orders, tone: "text-[#b8f7e4]" },
      { icon: ClipboardList, label: "Активные", value: totals.activeOrders, tone: "text-[#efb56a]" },
      { icon: CalendarClock, label: "Будущие выезды", value: totals.visits, tone: "text-[#b8f7e4]" },
    ...
    ].map(({ icon: Icon, label, value, tone }) => <div key={label} className="bg-[#2b2e34] p-4"><Icon className={`size-4 ${tone}`} /><p className="mt-4 text-[9px] uppercase tracking-[0.12em] text-[#5f6a70]">{label}</p><p className="mt-1 font-display text-lg font-semibold text-white">{value}</p></div>)}</div></section> : null}

    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{organizations.map((organization) => <article key={organization.id} className={`surface-panel overflow-hidden ${organization.current ? "border-[var(--accent)]/20 bg-[var(--accent)]/[0.025]" : ""}`}><div className="p-5"><div className="flex items-start justify-between gap-3"><span className={`grid size-10 place-items-center rounded-[12px] ${organization.kind === "center" ? "bg-[var(--accent)]/[0.08] text-[var(--accent)]" : "bg-[#b8f7e4]/[0.08] text-[#b8f7e4]"}`}>{organization.kind === "center" ? <Building2 className="size-4" /> : <Globe2 className="size-4" />}</span><OpenOrganizationButton organization={organization} /></div><h3 className="mt-4 text-sm font-medium text-white">{organization.name}</h3><p className="mt-1 text-[10px] text-[#667178]">{organization.kind === "center" ? "Управление группой компаний" : "Изолированное рабочее пространство"}</p></div><dl className="grid grid-cols-3 gap-px bg-white/[0.05] text-center"><div className="bg-[#2b2e34] px-2 py-3"><dt className="text-[8px] uppercase text-[#59656b]">Клиенты</dt><dd className="mt-1 text-xs text-white">{organization.clientCount}</dd></div><div className="bg-[#2b2e34] px-2 py-3"><dt className="text-[8px] uppercase text-[#59656b]">Заказы</dt><dd className="mt-1 text-xs text-white">{organization.orderCount}</dd></div><div className="bg-[#2b2e34] px-2 py-3"><dt className="text-[8px] uppercase text-[#59656b]">Активные</dt><dd className="mt-1 text-xs text-white">{organization.activeOrderCount}</dd></div></dl></article>)}</section>
