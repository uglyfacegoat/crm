...
export function AppearancePanel({ fontScale, digitStyle }: { fontScale: FontScale; digitStyle: DigitStyle }) {
  ...
  return <form action={action} className="mt-5 grid gap-4 xl:grid-cols-2">
    <section className="surface-panel p-5 sm:p-6">
      <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-[12px] bg-[#65b7ee]/[0.08] text-[#65b7ee]"><TextCursorInput className="size-4" /></span><div><h2 className="font-display text-base font-semibold text-white">Размер интерфейса</h2><p className="mt-1 text-xs leading-5 text-[#737e84]">Настройка применяется ко всем разделам после сохранения.</p></div></div>
    ...
    <section className="surface-panel p-5 sm:p-6">
      <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-[12px] bg-[#a892ec]/[0.08] text-[#a892ec]"><Rows3 className="size-4" /></span><div><h2 className="font-display text-base font-semibold text-white">Отображение чисел</h2><p className="mt-1 text-xs leading-5 text-[#737e84]">Табличный вариант удобнее для сумм, KPI и реестров.</p></div></div>
    ...
    <div className="flex flex-col gap-3 rounded-[15px] border border-white/[0.07] bg-[#10171b] p-4 sm:flex-row sm:items-center sm:justify-between xl:col-span-2"><p role="status" className={`text-xs ${state.status === "error" ? "text-[#df8589]" : "text-[#778188]"}`}>{state.message ?? "Выбор хранится для текущего браузера и не меняет данные компании."}</p><button type="submit" disabled={pending} className="focus-ring min-h-11 rounded-[13px] bg-[var(--accent)] px-5 text-xs font-semibold text-[#111509] disabled:opacity-50">{pending ? "Сохраняю…" : "Сохранить представление"}</button></div>
  </form>;
}
