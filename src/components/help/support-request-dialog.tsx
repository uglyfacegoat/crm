...
function SupportForm({ onComplete }: { onComplete: () => void }) {
  ...
  return <form action={action} className="flex flex-1 flex-col"><input type="hidden" name="idempotencyKey" value={idempotencyKey} /><div className="flex-1 space-y-5 p-5 sm:p-7">
    ...
    <Field label="Подробности *" error={state.fieldErrors.description}><textarea name="description" required minLength={20} maxLength={4000} placeholder="Что вы делали, что произошло, какой результат ожидали. Не указывайте пароль или секретный ключ." className={`${inputClass} min-h-40 p-3 leading-5`} /></Field>
