...
function UploadDocumentVersionForm({ document, requestKey, onComplete }: { document: DocumentListItem; requestKey: string; onComplete: () => void }) {
  ...
  return <form action={formAction} className="flex min-h-full flex-1 flex-col">
    ...
    <div className="flex-1 space-y-5 p-5 sm:p-7">
      <div className="grid gap-3 rounded-[15px] border border-white/[0.07] bg-black/15 p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center">
        <span className="grid size-11 place-items-center rounded-[13px] bg-[var(--accent)]/[0.08] text-[var(--accent)]"><FileClock className="size-5" /></span>
        <div className="min-w-0"><p className="truncate text-sm font-medium text-white">{document.title}</p><p className="mt-1 truncate text-[10px] text-[#6e787e]">{document.filename}</p></div>
        <div className="rounded-[10px] border border-[var(--accent)]/15 bg-[var(--accent)]/[0.05] px-3 py-2 text-center"><p className="text-[9px] uppercase tracking-[0.12em] text-[#7d865b]">Следующая</p><p className="mt-1 font-display text-lg font-semibold text-[var(--accent)]">v{document.versionNumber + 1}</p></div>
      </div>
      ...
      <OrderField label="Новый файл до 15 МБ" required errors={state.fieldErrors.file}>
        <span className="focus-within:focus-ring flex min-h-32 cursor-pointer flex-col items-center justify-center rounded-[15px] border border-dashed border-white/[0.12] bg-black/15 px-5 py-5 text-center hover:border-[var(--accent)]/30 hover:bg-[var(--accent)]/[0.025]">
          <UploadCloud className="size-6 text-[var(--accent)]" />
          <span className="mt-2 text-xs font-medium text-white">PDF, изображение, DOCX или XLSX</span>
          <span className="mt-1 text-[10px] leading-4 text-[#687279]">Текущая версия останется доступна в истории</span>
...
export function UploadDocumentVersionButton({ document }: { document: DocumentListItem }) {
  ...
  return <>
    <button type="button" onClick={() => setRequestKey(crypto.randomUUID())} className="focus-ring flex h-11 flex-1 items-center justify-center gap-2 rounded-[12px] border border-white/[0.08] bg-white/[0.025] px-3 text-xs font-medium text-[#d3d9d5] hover:bg-white/[0.05]"><RefreshCw className="size-4 text-[var(--accent)]" />Новая версия</button>
