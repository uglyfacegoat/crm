...
function UploadDocumentForm({
  ...
  const visits = useMemo(
    () => options.visits.filter((visit) => visit.orderId === orderId),
    [options.visits, orderId],
  );
  const selectedOrder =
    options.orders.find((order) => order.id === orderId) ?? null;
  const contracts = useMemo(
    () =>
      options.contracts.filter(
        (contract) => contract.objectId === selectedOrder?.objectId,
      ),
  ...
  return (
    <form action={formAction} className="flex min-h-full flex-1 flex-col">
      ...
      <input
        type="hidden"
        name="contractId"
        value={category === "contract" ? contractId : ""}
      />
      ...
      <div className="flex-1 space-y-5 p-5 sm:p-7">
        <OrderPicker
          ...
          errors={state.fieldErrors.orderId}
          options={options.orders.map((order) => ({
            value: order.id,
            label: `№${order.number} · ${order.client}`,
            detail: `${order.object} · ${order.address}`,
          }))}
          searchable
        ...
        <OrderPicker
          ...
          placeholder={
            orderId
              ? "Без привязки к конкретному выезду"
              : "Сначала выберите заказ"
          }
          disabled={!orderId}
          errors={state.fieldErrors.visitId}
          options={[
            { value: "", label: "Без конкретного выезда" },
            ...visits.map((visit) => ({
              value: visit.id,
              label: visitDateFormatter.format(
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <OrderPicker
            ...
            onChange={(value) => {
              ...
              if (nextCategory !== "contract") setContractId("");
            }}
            placeholder="Выберите категорию"
            errors={state.fieldErrors.category}
            options={(
              Object.entries(documentCategoryLabels) as [
                DocumentCategory,
                string,
              ][]
        ...
        <OrderField label="Описание" errors={state.fieldErrors.description}>
          <textarea
            name="description"
            maxLength={2000}
            placeholder="Что важно знать об этом файле"
        ...
        >
          <span className="focus-within:focus-ring flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-[14px] border border-dashed border-[var(--line-strong)] bg-[var(--surface-inset)] px-5 py-5 text-center hover:border-[var(--accent)]/45 hover:bg-[var(--accent-soft)]">
            <UploadCloud className="size-6 text-[var(--accent)]" />
            <span className="mt-2 text-xs font-medium text-[var(--text)]">
              Выберите PDF, изображение, DOCX или XLSX
            </span>
            <span className="mt-1 text-[10px] text-[var(--muted)]">
              Тип и содержимое проверяются на сервере
            </span>
            <input
              name="file"
              type="file"
              ...
              className="mt-3 block max-w-full text-[10px] text-[var(--muted)] file:mr-3 file:rounded-lg file:border-0 file:bg-[var(--surface-soft)] file:px-3 file:py-2 file:text-[10px] file:text-[var(--text-secondary)]"
            />
          </span>
        </OrderField>
      ...
      {category === "contract" ? (
        <OrderPicker
          ...
          placeholder={
            orderId
              ? "Выберите договор для быстрого просмотра"
              : "Сначала выберите заказ"
          }
          disabled={!orderId}
          errors={state.fieldErrors.contractId}
          options={[
            { value: "", label: "Без привязки к договору" },
            ...contracts.map((contract) => ({
              value: contract.id,
              label: contract.contractNumber,
              detail: `${contract.clientName} · ${contract.objectName}`,
            })),
          ]}
          searchable
          searchPlaceholder="Номер договора или клиент"
        />
      ) : null}
      <OrderFormFooter
        pending={pending}
...
export function EmptyDocumentsAction({
  options,
}: {
  options: DocumentUploadOptions;
}) {
  return (
    <div className="grid justify-items-center text-center">
      <span className="grid size-12 place-items-center rounded-[15px] bg-[var(--accent-soft)] text-[var(--accent)]">
      ...
      <p className="mt-1 max-w-sm text-xs leading-5 text-[var(--muted)]">
        Добавьте договор, акт, фото или карточку выезда — связи с заказчиком
        сохранятся автоматически.
      </p>
      <div className="mt-5">
        <UploadDocumentButton options={options} />
      </div>
    </div>
  );
}
