    ...
    <section className="surface-panel animate-rise" style={{ animationDelay: "240ms" }}>
      <div className="flex items-center justify-between border-b border-white/[0.06] px-[clamp(1rem,0.75rem+0.5vw,1.5rem)] py-4 2xl:py-5">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--muted)]">Сегодня, 25 августа</p>
        </div>
      ...
      <div className="divide-y divide-white/[0.055]">
        {visits.map((visit, index) => (
          <Link key={visit.id} href={`/orders/${visit.orderId}`} className="focus-ring group grid grid-cols-[3.25rem_minmax(0,1fr)_auto] items-center gap-3 px-[clamp(1rem,0.75rem+0.5vw,1.5rem)] py-3.5 transition-colors duration-200 hover:bg-white/[0.035] 2xl:py-4">
