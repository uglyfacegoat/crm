...
function EditTaskForm({ task, assigneeOptions, timeZone, onComplete }: { task: TaskCard; assigneeOptions: TaskAssigneeOption[]; timeZone: string; onComplete: () => void }) {
  ...
  return <form action={action} className="flex flex-1 flex-col">
    ...
    <div className="flex-1 space-y-5 p-5 sm:p-7">
      ...
      <label className="grid gap-2 text-[10px] text-[#7b858b]"><span>Ответственный</span><select name="assignedMemberId" defaultValue={task.assignedMemberId ?? ""} className={`${fieldClass} bg-[#202227]`}><option value="">Без ответственного</option>{assigneeOptions.map((assignee) => <option key={assignee.id} value={assignee.id}>{assignee.displayName} · {roleLabels[assignee.role]}</option>)}</select><FieldError errors={state.fieldErrors.assignedMemberId} /></label>
      ...
      <fieldset disabled={managedByVisit}><legend className="text-[10px] text-[#7b858b]">Срок</legend><div className="mt-2 grid gap-3 sm:grid-cols-2"><label className="grid gap-2 text-[10px] text-[#69737a]"><span>Дата</span><DateInput name={managedByVisit ? undefined : "localDate"} defaultValue={deadline.date} className={`${fieldClass} [color-scheme:dark]`} /><FieldError errors={state.fieldErrors.localDate} /></label><label className="grid gap-2 text-[10px] text-[#69737a]"><span>Время</span><TimeInput name={managedByVisit ? undefined : "localTime"} defaultValue={deadline.time} className={`${fieldClass} [color-scheme:dark]`} /><FieldError errors={state.fieldErrors.localTime} /></label></div></fieldset>
