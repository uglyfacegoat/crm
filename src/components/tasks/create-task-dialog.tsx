...
function CreateTaskForm({ requestKey, assigneeOptions, currentMemberId, onComplete }: { requestKey: string; assigneeOptions: TaskAssigneeOption[]; currentMemberId: string; onComplete: () => void }) {
  ...
  return <form action={action} className="flex flex-1 flex-col">
    ...
    <div className="flex-1 space-y-5 p-5 sm:p-7">
      ...
      <label className="grid gap-2 text-[10px] text-[#7b858b]"><span>Ответственный</span><select name="assignedMemberId" defaultValue={currentMemberId} className="focus-ring h-12 rounded-[12px] border border-white/[0.08] bg-[#202227] px-3.5 text-sm text-white outline-none"><option value="">Без ответственного</option>{assigneeOptions.map((assignee) => <option key={assignee.id} value={assignee.id}>{assignee.displayName} · {roleLabels[assignee.role]}</option>)}</select><FieldError errors={state.fieldErrors.assignedMemberId} /></label>
      ...
      <fieldset><legend className="text-[10px] text-[#7b858b]">Срок (необязательно)</legend><div className="mt-2 grid gap-3 sm:grid-cols-2"><label className="grid gap-2 text-[10px] text-[#69737a]"><span>Дата</span><DateInput name="localDate" className="focus-ring h-12 rounded-[12px] border border-white/[0.08] bg-black/15 px-3.5 text-sm text-white outline-none [color-scheme:dark]" /><FieldError errors={state.fieldErrors.localDate} /></label><label className="grid gap-2 text-[10px] text-[#69737a]"><span>Время</span><TimeInput name="localTime" className="focus-ring h-12 rounded-[12px] border border-white/[0.08] bg-black/15 px-3.5 text-sm text-white outline-none [color-scheme:dark]" /><FieldError errors={state.fieldErrors.localTime} /></label></div></fieldset>
      {state.message ? <p role="status" className={`rounded-[12px] border p-3 text-xs ${state.status === "success" ? "border-[#b8f7e4]/20 bg-[#b8f7e4]/[0.05] text-[#8ed7b8]" : "border-[#ef646a]/20 bg-[#ef646a]/[0.05] text-[#d89599]"}`}>{state.status === "success" ? <Check className="mr-2 inline size-4" /> : null}{state.message}</p> : null}
    </div>
    <footer className="sticky bottom-0 flex gap-2 border-t border-white/[0.07] bg-[#25272c]/94 p-4 backdrop-blur-xl sm:px-7"><button type="button" onClick={onComplete} disabled={pending} className="focus-ring h-12 flex-1 rounded-[12px] border border-white/[0.08] text-xs text-[#8b959b]">Отмена</button><button type="submit" disabled={pending || state.status === "success"} className="focus-ring flex h-12 flex-[1.4] items-center justify-center gap-2 rounded-[12px] bg-[var(--accent)] text-xs font-semibold text-[#25272c] disabled:opacity-65">{pending ? <><LoaderCircle className="size-4 animate-spin" />Сохраняем…</> : state.status === "success" ? <><Check className="size-4" />Сохранено</> : "Создать задачу"}</button></footer>
  </form>;
}
