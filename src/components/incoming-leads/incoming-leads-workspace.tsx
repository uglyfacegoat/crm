...
const statusTone: Record<IncomingLeadStatus, string> = {
  ...
  reviewing: "border-[#b8f7e4]/20 bg-[#b8f7e4]/[0.07] text-[#b8f7e4]",
  accepted: "border-[#b8f7e4]/20 bg-[#b8f7e4]/[0.07] text-[#7ad5ad]",
  ...
  return <Dialog open={open} onClose={onClose} title="Отклонить заявку" description="Она останется в истории и не превратится в заказ.">
    <form action={action} className="flex min-h-full flex-1 flex-col">
      ...
      <div className="flex-1 space-y-5 p-5 sm:p-7">
        ...
        {state.message ? <p role="status" className={`rounded-[12px] border p-3 text-xs leading-5 ${state.status === "success" ? "border-[#b8f7e4]/20 bg-[#b8f7e4]/[0.05] text-[#83d4b1]" : "border-[#ef646a]/20 bg-[#ef646a]/[0.05] text-[#d98b90]"}`}>{state.message}</p> : null}
      </div>
      <footer className="sticky bottom-0 mt-auto grid grid-cols-2 gap-2 border-t border-white/[0.07] bg-[#25272c]/95 p-4 backdrop-blur-xl sm:p-5">
...
function LeadDetails({ lead, canWrite, onReject }: { lead: IncomingLead; canWrite: boolean; onReject: () => void }) {
  ...
  return <article className="flex min-h-[33rem] flex-col overflow-hidden rounded-[18px] border border-white/[0.075] bg-[#2b2e34]">
    ...
    <div className="grid flex-1 content-start gap-6 p-5 sm:p-6">
      ...
      {lead.possibleClientId ? <section className="rounded-[14px] border border-[#b8f7e4]/15 bg-[#b8f7e4]/[0.045] p-4"><div className="flex gap-3"><UserRoundSearch className="size-4 shrink-0 text-[#b8f7e4]" /><div><p className="text-xs font-medium text-[#cbdde8]">В CRM найден возможный клиент</p><Link href={`/clients/${lead.possibleClientId}`} className="focus-ring mt-1 inline-flex items-center gap-1 text-[10px] text-[#b8f7e4] hover:text-white">{lead.possibleClientName}<ArrowRight className="size-3" /></Link><p className="mt-2 text-[9px] leading-4 text-[#68808d]">При оформлении он будет выбран автоматически. Проверьте совпадение контакта.</p></div></div></section> : null}
...
export function IncomingLeadsWorkspace({ snapshot, filter, canWrite, preview }: { snapshot: IncomingLeadSnapshot; filter: IncomingLeadListFilter; canWrite: boolean; preview: boolean }) {
  ...
  return <div><PageHeading eyebrow="Первичный разбор" title="Входящие заявки" description="Новые обращения с сайтов сначала проверяются здесь. Только подтверждённая заявка создаёт клиента, объект, заказ и первый выезд." />
    <section className="mt-[clamp(1.5rem,1.1rem+0.8vw,2.25rem)] overflow-hidden rounded-[18px] border border-white/[0.075] bg-[linear-gradient(110deg,rgba(184,247,228,0.045),rgba(184,247,228,0.025)_48%,rgba(184,247,228,0.03))]">
