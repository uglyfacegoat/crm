...
export async function readinessResponse() {
  try {
    ...
    console.error(JSON.stringify({ operation: "system.readiness", category: "database_unavailable" }));
    return Response.json({ status: "unavailable", service: "crm-web", database: "unavailable" }, { status: 503, headers });
  }
}
