...
export async function getWebsiteSnapshot(member: AuthenticatedMember): Promise<WebsiteSnapshot> {
  ...
  return {
    ...
    trafficTrend: { labels: dates.map(formatDateLabel), series: [
      { label: "Посетители", color: "#b8f7e4", values: dates.map((date) => trafficByDate.get(date)?.visitors ?? 0), valueFormat: "integer" },
      { label: "Просмотры", color: "#9ca5a3", values: dates.map((date) => trafficByDate.get(date)?.pageviews ?? 0), valueFormat: "integer" },
      { label: "Поисковые клики", color: "#f4f7f6", values: dates.map((date) => searchClicksByDate.get(date) ?? 0), valueFormat: "integer" },
    ] },
