...
import type { CreateTaskInput, RescheduleTaskInput, TaskMutationInput } from "./schemas";
import type { TaskCard, TaskColumn, TaskSnapshot } from "./types";
...
export async function createTask(member: AuthenticatedMember, input: CreateTaskInput) {
  ...
  return sql.begin(async (transaction) => {
    const [due] = input.localDate && input.localTime
      ...
      : [{ due_at: null }];
    const [task] = await transaction`INSERT INTO tasks (
      ...
      ${member.memberId}, 'manual', ${input.idempotencyKey}, ${member.memberId}, ${member.memberId}
    ...
    await transaction`INSERT INTO audit_events (organization_id, actor_id, auth_session_id, action, entity_type, entity_id, changes)
      VALUES (${member.organizationId}, ${member.memberId}, ${member.sessionId}, 'task.create', 'task', ${taskId},
        ${transaction.json({ title: input.title, priority: input.priority, dueAt: due?.due_at ?? null })})`;
    return taskId;
  });
}
...
export async function completeTask(member: AuthenticatedMember, input: TaskMutationInput) {
  ...
  return sql.begin(async (transaction) => {
    const [task] = await transaction`UPDATE tasks SET status = 'completed', completed_at = now(), completed_by = ${member.memberId},
      ...
      WHERE organization_id = ${member.organizationId} AND id = ${input.taskId} AND version = ${input.expectedVersion} AND status = 'open'
      RETURNING version`;
    if (!task) {
      const existing = await transaction`SELECT version FROM tasks WHERE organization_id = ${member.organizationId} AND id = ${input.taskId}`;
      if (!existing.length) throw new TaskNotFoundError();
    ...
    return z.number().int().positive().parse(task.version);
  });
}
  ...
  return sql.begin(async (transaction) => {
    ...
    await transaction`INSERT INTO audit_events (organization_id, actor_id, auth_session_id, action, entity_type, entity_id, changes)
      VALUES (${member.organizationId}, ${member.memberId}, ${member.sessionId}, 'task.reschedule', 'task', ${input.taskId},
        ${transaction.json({ column: input.column, dueAt: schedule?.due_at ?? null })})`;
    return z.number().int().positive().parse(task.version);
  });
}
