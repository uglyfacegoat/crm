...
const workingDays = z.array(z.coerce.number().int().min(1).max(7)).min(1, "Выберите хотя бы один рабочий день").transform((value) => [...new Set(value)].sort());
const statusUntil = z.union([z.literal(""), z.iso.date()]).transform((value) => value || null);
...
function validateAvailability(
  value: { operationalStatus: (typeof masterOperationalStatuses)[number]; statusNote: string | null },
  context: z.RefinementCtx,
) {
  if (value.operationalStatus !== "working" && !value.statusNote) {
    context.addIssue({ code: "custom", path: ["statusNote"], message: "Укажите причину статуса" });
  }
}
...
export const updateMasterSchema = z.object({
  ...
  if (value.operationalStatus === "terminated" && value.active) context.addIssue({ code: "custom", path: ["active"], message: "Уволенный мастер не может быть активным" });
