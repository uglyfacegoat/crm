...
export async function startS3Fixture() {
  ...
  try {
    execFileSync("docker", ["run", "--detach", "--name", name, "--add-host", "host.docker.internal:host-gateway", "--publish", "127.0.0.1::9000", "--tmpfs", "/data:rw,size=128m", "--env", "MINIO_ROOT_USER", "--env", "MINIO_ROOT_PASSWORD", image, "server", "/data"], {
