...
try {
  ...
  const memberId = await sql.begin(async (transaction) => {
    ...
    const [member] = await transaction`INSERT INTO organization_members (organization_id, display_name, email, role)
      VALUES (${environment.AUTH_MEMBER_ORGANIZATION_ID}, ${environment.AUTH_MEMBER_NAME}, ${emailIdentity.normalizedValue}, ${environment.AUTH_MEMBER_ROLE})
      RETURNING id`;
